...
async function loadTasks(token) {
    try {
        const response = await fetch('https://todo-backend-a972.onrender.com/tasks', {
            method: 'GET',
            headers: {
...
async function editTask(token, taskId) {
    ...
    try {
        const response = await fetch(`https://todo-backend-a972.onrender.com/tasks/${taskId}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
...
async function deleteTask(id) {
    ...
    if (confirm('Tem certeza que deseja deletar esta tarefa?')) {
        try {
            const response = await fetch(`https://todo-backend-a972.onrender.com/tasks/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
